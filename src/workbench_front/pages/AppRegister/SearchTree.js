    ...
    render() {
        ...
        const loop = (data) =>
            data.map((item) => {
                ...
                return <TreeNode icon={<span className='tree-dot' />} key={code} title={title} refData={item} />;
            });
        let newTreeData = [
            {
                /* 给树填个根 */
                systypename: langCheck('102202APP-000144', 'pages', langJson) /* 国际化处理： 应用节点*/,
                moduleid: '00',
                children: createTree(this.props.treeData, 'moduleid', 'parentcode')
            }
        ];
        return (
            <div className={this.props.className}>
                <div className='fixed-search-input'>
                    <Input
                        placeholder={langCheck('102202APP-000086', 'pages', langJson)} /* 国际化处理： 应用查询*/
                        value={searchValue}
